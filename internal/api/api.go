...
func (s *Server) downloadReport(w http.ResponseWriter, r *http.Request) {
	...
	// Load data once; branch on format at the end.
	var (
		out *report.Output
		err error
	...
	// already validated the session; re-read the cookie and look up the
	// user without writing errors (the byline is optional — we fall back
	// to empty if anything goes wrong).
	generatedBy := ""
	if c, err := r.Cookie("cv_session"); err == nil && c.Value != "" {
		_, u, err := s.DB.LookupSession(c.Value)
		if err == nil && u != nil {
	...
	case "installed_software":
		if host != "" {
			softwareRows, err = s.DB.InstalledSoftwareForHost(host)
		} else {
			softwareRows, err = s.DB.AllInstalledSoftware()
		}
		if err != nil {
			break
		}
	...
	case "local_admins":
		adminsRows, err = s.DB.LocalAdminsAllHosts()
		if err != nil {
			break
		}
		switch format {
	...
	case "cve_findings":
		if host != "" {
			cveRows, err = s.DB.CVEFindingsForHost(host)
		} else {
			cveRows, err = s.DB.AllCVEFindings()
		}
		if err != nil {
			break
		}
	...
	// HTML renders inline so the browser can open it for print-to-PDF; CSV
	// and JSON are downloads.
	if format == report.FormatHTML {
		w.Header().Set("Content-Disposition", fmt.Sprintf(`inline; filename="%s"`, out.Filename))
	} else {
